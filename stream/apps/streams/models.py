...
class Stream(models.Model):
    ...
    auto_now_add=True)
    streamer = models.ForeignKey('accounts.User', on_delete=models.CASCADE,
                                 related_name='user_streams')
    game = models.ForeignKey('games.Games', on_delete=models.CASCADE,
                             related_name='game_streams')
    live = models.BooleanField('В эфире', default=False)
    live_url = models.URLField('Ссылка на стрим',
                blank=True, null=True)
    viewers = models.PositiveIntegerField('Просмотры',
                    default=0)
